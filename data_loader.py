...
from config import HEALTHY_PATH, SICK_PATH


def load_data():
    """
    Load data_cbio from csv files.
...
def load_np_data():
    """
    Loads data_cbio from csv files and converts them to numpy arrays.
...
def load_merged_data():
    ...
    train = train.drop(columns=[train.columns[0]]).to_numpy()
    train_healthy = train[train[:, -1] == 0][:, :-1]
    train_sick = train[train[:, -1] == 1][:, :-1]

    test = pd.read_csv(TEST_PATH)
    test = test.drop(columns=[test.columns[0]]).to_numpy()
